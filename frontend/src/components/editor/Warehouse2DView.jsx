 ...
 * ✅ Ahora con:
 * - Cotas como antes
 * - Drag & drop REAL en SVG (sin Moveable → cero bugs de “prohibido”)
...
function Zone2D({
  ...
  onMouseLeave,
  onClick,
  onPointerDownZone // ✅ drag start (solo para movibles)
}) {
...
export default function Warehouse2DView({
  ...
  showGrid = true,
  showDimensions = true,
  zoom = 100, // (lo mantiene el padre, aquí no lo necesitamos para convertir)
  onElementMoveEnd
}) {
  ...
  const onPointerDownZone = useCallback((zone, e) => {
    ...
    // Evita drag nativo del navegador
    e.preventDefault()
    e.stopPropagation()

    // Selecciona el elemento al empezar el drag
    setInternalSelected(zone.id)
    onZoneSelect?.(zone)

    const g = e.currentTarget // <g> del zone
    ...
    const onMove = (ev) => {
      ...
      // mover visualmente con transform en unidades SVG
