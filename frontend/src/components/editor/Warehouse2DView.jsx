 ...
 * - ✅ FIX DEDUP: Prioriza zonas locales sobre backend para mantener element
 *
 * @version 2.3.5 - FIX DEDUPLICACIÓN DE ZONAS
 */
...
// ============================================================
// DEBUG - FORZADO SIEMPRE PARA DIAGNOSTICAR
// ============================================================
const DEBUG_2D = true // ✅ FORZADO A TRUE
const dlog = (...args) => console.log(...args) // ✅ SIEMPRE LOG
const dwarn = (...args) => console.warn(...args)
const derr = (...args) => console.error(...args)
...
const AUTO_GENERATED_TYPES = new Set([
  'aisle',
  'main_aisle',
  'cross_aisle',
  'circulation',
...
function Zone2D({
  ...
  // ✅ si scale fuese raro (0/negativo), NO dibujamos
  ...
  // ✅ Handler local para interceptar el pointerDown
  const handlePointerDown = (e) => {
    console.log('🖱️ [Zone2D] handlePointerDown FIRED!', {
      id: zone.id,
      type: zone.type,
      draggable,
      button: e.button,
      hasElement: !!zone.element,
      isAutoGenerated: zone.isAutoGenerated
    })
    
    if (draggable && onPointerDownZone) {
      onPointerDownZone(zone, e)
    }
  }
...
function DimensionLines({ dimensions, scale, offset }) {
  ...
  // ✅ si scale está mal, no dibujes cotas
  if (!(svgLength > 0) || !(svgWidth > 0)) return null
...
function processElementsToZones(elements, dimensions) {
  ...
  ;(elements || []).forEach((el, index) => {
    ...
    const zoneData = {
      ...
      area: width * height,
      element: el,
      isAutoGenerated: false // ✅ Marcar explícitamente como NO auto-generado
    }
    ...
    if (type === 'dock') {
      ...
      zones.push({
        ...
        element: el,
        isAutoGenerated: false // Las zonas de maniobra pertenecen al dock
      })
    }
  })
  ...
  const totalArea = L * W
  const occupiedArea = zones.reduce((sum, z) => sum + (z.area || 0), 0)
...
function clampRectToBounds(r, L, W) {
...
function detectAisles(shelves, dimensions, allElements) {
  ...
  const obstacles = obstaclesRaw
    .map(r => clampRectToBounds(r, L, W))
    .filter(Boolean)

  const yPoints = new Set([0, W])
...
export default function Warehouse2DView({
  ...
  // ============================================================
  // ✅ FIX CRÍTICO: DEDUPLICAR ZONAS
  // Priorizar zonas locales (con element) sobre las del backend
  // ============================================================
  const zones = useMemo(() => {
    if (externalZones && externalZones.length > 0) {
      // Zonas de elementos (locales) - SIEMPRE tienen element
      const elementZones = localZones.filter(z => !z.isAutoGenerated)
      
      // IDs de las zonas de elementos para evitar duplicados
      const elementIds = new Set(elementZones.map(z => z.id))
      
      // También incluir IDs de dock_maneuver que son derivados de docks
      elementZones.forEach(z => {
        if (z.type === 'dock') {
          elementIds.add(`${z.id}-maneuver`)
        }
      })
      
      // Solo incluir zonas del backend que:
      // 1. Sean auto-generadas (pasillos, circulación)
      // 2. NO tengan el mismo ID que una zona de elemento local
      // 3. Sean de tipos que realmente son auto-generados
      const backendAutoZones = externalZones.filter(z => {
        const isAutoType = AUTO_GENERATED_TYPES.has(z.type)
        const notDuplicate = !elementIds.has(z.id)
        return isAutoType && notDuplicate
      })
      ...
      console.log('🔧 [2DView] Zonas combinadas:', {
        elementZones: elementZones.length,
        backendAutoZones: backendAutoZones.length,
        total: result.length,
        elementIds: [...elementIds].slice(0, 5)
      })
  ...
  // ============================================================
  // ✅ FIX SCALE/OFFSET: scale SIEMPRE POSITIVO
  // ============================================================
  ...
  useEffect(() => {
    const updateSize = () => {
      if (containerRef.current) {
        ...
        dlog('[2DView][resize] container:', rect.width, rect.height)
      }
    }
    updateSize()
  ...
  const onPointerDownZone = useCallback((zone, e) => {
    console.log('🎯 [2DView] onPointerDownZone CALLED!', {
      zoneId: zone.id,
      zoneType: zone.type,
      button: e.button,
      hasElement: !!zone.element
    ...
    if (e.button !== 0) {
      console.log('❌ [2DView] Button !== 0, ignorando')
      return
    }
    ...
    console.log('🔍 [2DView] draggable check:', {
      hasElement: !!zone.element,
      isAutoGenerated: zone.isAutoGenerated,
      type: zone.type,
      inMovableTypes: MOVABLE_TYPES.has(zone.type),
      result: draggable
    })

    if (!draggable) {
      console.log('❌ [2DView] Zona NO movible:', { id: zone.id, type: zone.type })
      return
    }
    ...
    const onUp = (ev) => {
      ...
      console.log('🏁 [2DView][drag] END -> onElementMoveEnd', {
        id: d.elementId,
        x: d.lastX,
        y: d.lastY,
        totalMoves: d.moves
    ...
    console.log('👂 [2DView] Registrando listeners globales (pointermove, pointerup)')
  ...
  useEffect(() => {
    const invalid = zones.filter(z => !(z.width > 0) || !(z.height > 0))
    if (invalid.length) {
      derr('[2DView] ZONAS INVÁLIDAS (no deberían existir). sample:', invalid.slice(0, 10))
    } else {
      dlog('[2DView] zones OK:', zones.length, 'scale:', scale)
    }
  }, [zones, scale])

  const safeScale = Number.isFinite(scale) && scale > 0 ? scale : 0.01
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showGrid && (
          <Grid2D dimensions={dimensions} scale={safeScale} offset={offset} gridSize={5} />
        )}

        <rect
          x={offset.x}
          y={offset.y}
          width={toPosNumber(dimensions.length, 80) * safeScale}
          height={toPosNumber(dimensions.width, 40) * safeScale}
          fill="#ffffff"
          stroke="#374151"
          strokeWidth={3}
          filter="url(#shadow)"
        ...
        {showDimensions && (
          <DimensionLines dimensions={dimensions} scale={safeScale} offset={offset} />
        )}
        ...
        <g transform={`translate(${offset.x}, ${offset.y + toPosNumber(dimensions.width, 40) * safeScale + 60})`}>
          <rect x={0} y={0} width={10 * safeScale} height={6} fill="#374151" />
          <rect x={10 * safeScale} y={0} width={10 * safeScale} height={6} fill="#94a3b8" />
          <text x={0} y={18} fontSize={9} fill="#64748b">0</text>
          <text x={10 * safeScale} y={18} fontSize={9} fill="#64748b" textAnchor="middle">10m</text>
          <text x={20 * safeScale} y={18} fontSize={9} fill="#64748b" textAnchor="middle">20m</text>
        </g>
        ...
        <text
          ...
          fill="#1f2937"
        >
          PLANO DE PLANTA - {toPosNumber(dimensions.length, 80)}m × {toPosNumber(dimensions.width, 40)}m ({(toPosNumber(dimensions.length, 80) * toPosNumber(dimensions.width, 40)).toLocaleString()}m²)
        </text>
      </svg>
