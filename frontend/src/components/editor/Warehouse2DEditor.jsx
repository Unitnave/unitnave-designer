 ...
 * 
 * @version 2.0 - Con geometría exacta
 */
...
function useBackendZones(dimensions, elements) {
  ...
  useEffect(() => {
    // Si no hay elementos, no llamar al backend
    ...
    const fetchZones = async () => {
      ...
      try {
        ...
        // Convertir zonas del backend al formato esperado
        ...
        console.warn('⚠️ Backend no disponible, usando algoritmo local:', err.message)
...
export default function Warehouse2DEditor({
  ...
  // Obtener zonas del backend (geometría exacta)
  const { backendZones, metrics, loading, error } = useBackendZones(dimensions, elements)
  ...
  // Usar backend si disponible, sino local
  const zones = useMemo(() => {
    if (backendZones && backendZones.length > 0) {
      console.log('✅ Usando geometría exacta del backend')
      // Combinar zonas de elementos + zonas auto-generadas del backend
  ...
  // Métricas (backend o calculadas)
  const displayMetrics = useMemo(() => {
    if (metrics) {
      return metrics
    }
    // Fallback: calcular desde zonas locales
    const totalArea = dimensions.length * dimensions.width
        ...
        >
          {loading && (
            <Box sx={{ 
              ...
              <Typography variant="caption">Calculando geometría exacta...</Typography>
            </Box>
          )}
          <Warehouse2DView
