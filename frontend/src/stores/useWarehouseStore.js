...
import { TransformControls, Html, OrthographicCamera, PerspectiveCamera } from '@react-three/drei'
...
export default function Warehouse3D() {
  ...
  const { viewMode, cameraType, selectedElement, selectElement, previewElement, showDistances } = useUIStore()
  ...
  useEffect(() => {
    ...
    if (cameraType === 'orthographic' && orthoCameraRef.current) {
    ...
    switch(viewMode) {
      case 'Planta':
        camera.position.set(length/2, height * 3, width/2)
        camera.lookAt(length/2, 0, width/2)
        camera.up.set(0, 0, -1)
        break
      case 'Alzado':
        camera.position.set(length/2, height/2, width * 2.5)
        camera.lookAt(length/2, height/2, 0)
        camera.up.set(0, 1, 0)
        break
      case 'Perfil':
        camera.position.set(length * 2.5, height/2, width/2)
  ...
  }, [viewMode, cameraType, dimensions, camera])

  const canEdit = viewMode !== '3D'
  ...
  return (
    <>
      ...
      <mesh 
        ...
        receiveShadow
        onClick={() => canEdit && selectElement(null)}
      >
      ...
      {previewElement && canEdit && <PreviewElement element={previewElement} />}
...
function Element3D({ element, isSelected, viewMode, pallet, showDistances, warehouseDimensions, canEdit }) {
